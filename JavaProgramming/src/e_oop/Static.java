...
public class Static {
	 ...
	 * - static을 붇인 변수는 객체간에 변수의 값을 공유한다.
	 * - static이 붙은 멤버의 명칭 : 클래스변수, 클래스 메서드
	 * - static이 붙지 않은 멤버의 명칭 : 인스턴스 변수, 인스턴스 메서드
	 * 
	 *   JVM 메모리 구조
	 * - Method Area(메서드 영역) : 클래스 멤버가 저장된다. (static이 붙으면)
	 * - Heap : 객체가 저장된다. (static이 안붙으면)
	 * - Call Stack(호출 스택) : 현재 호출되어 있는 메서드가 저장된다. 
	 */
	
	// 값을 공유하기 위해 static 을 붙인다.
	static int var; // 클래스 변수
